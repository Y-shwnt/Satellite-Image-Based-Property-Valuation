...
Satellite images and trained models are not included in the repository due to size 
...
The notebooks are designed to be executed sequentially. Each notebook builds on
artifacts generated by the previous step.
...
1. **1_preprocessing.ipynb**  
   ...
   - Creates stratified price bins for balanced sampling  

2. **2_tabular_model.ipynb**  
   - Trains a tabular-only baseline model using XGBoost  
   - Evaluates performance using standard regression metrics  

3. **3_image_model.ipynb**  
   - Extracts satellite image embeddings using a pretrained ResNet-18 model  
   - Evaluates an image-only regression model using XGBoost  

4. **4_fusion_model.ipynb**  
   - Combines tabular features with PCA-reduced image embeddings  
   - Trains a multimodal fusion model  
   - Applies SHAP and Grad-CAM for model explainability  

5. **5_generate_predictions.ipynb**  
   - Loads the trained fusion model  
   - Generates property price predictions on unseen data  
   - Saves predictions for downstream analysis or reporting  

---

## Outputs

- Model performance metrics are displayed within the notebooks.
- Generated plots (EDA, SHAP, Grad-CAM) are saved under `data/plots/`.
- Prediction outputs are saved as CSV files for further evaluation.

---

## Notes

- Raw data, processed datasets, satellite images, and trained model artifacts are
  not included in the repository.
- All results can be reproduced by running the notebooks in order with the
  appropriate data and API credentials.
